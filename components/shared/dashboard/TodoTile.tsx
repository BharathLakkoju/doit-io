...
export default function TodoTile({ task }: { task: TaskType }) {
  return (
    <>
      <div className="flex gap-1 justify-between place-items-end rounded-lg">
        <div className="flex flex-col gap-2 justify-between items-baseline">
          ...
          <div className="flex gap-1 flex-wrap justify-start">
            {task.tags.map((tag, index) => (
              <Badge
                key={index}
                className="inline-block bg-gray-200 rounded-md p-1 text-xs font-semibold text-gray-700 mr-2 mb-2 hover:bg-gray-300"
              >
                {tag}
              </Badge>
            ))}
